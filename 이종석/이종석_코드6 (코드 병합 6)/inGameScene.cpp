...
void inGameScene::update()
{
	...
	for (int i = 0; i < MAP_TILEY; i++)
	{
		for (int j = 0; j < MAP_TILEX; j++)
		{
			...
			if (_tiles[i * MAP_TILEX + j].obj == OBJ_BLOCKS)
			{
...
void inGameScene::render()
{
	...
	for (int i = 0; i < MAP_TILEY; i++)
	{
		for (int j = 0; j < MAP_TILEX; j++)
		{
			...
			if (_tiles[j].rc.left > WINSIZEX) continue;
			if (_tiles[j].rc.right < 0) continue;
			if (_tiles[i].rc.top > WINSIZEY) continue;
			if (_tiles[i].rc.bottom < 0) continue;
			...
			if (_tiles[i * MAP_TILEX + j].terrain == TR_WATER || _tiles[i * MAP_TILEX + j].obj == OBJ_BLOCKS || _tiles[i * MAP_TILEX + j].terrain == TR_WALL)
			{
				...
				if (INPUT->getToggleKey(VK_F1))
				{
					...
					if (_tiles[i * MAP_TILEX + j].obj == OBJ_BLOCKS || _tiles[i * MAP_TILEX + j].terrain == TR_WALL)
					{
