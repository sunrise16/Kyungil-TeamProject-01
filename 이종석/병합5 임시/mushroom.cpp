...
HRESULT mushroom::init(float x, float y)
{
	...
	ZeroMemory(&mushroom, sizeof(tagMushroom));
...
// 업데이트 (update)
void mushroom::update(int _scrollX, int _scrollY, bool isStart, bool playerOn)
{
	// 버섯 행동
	for (int i = 0; i < _mushroom.size(); i++)
	{

		_mushroom[i].y += (scrollY - _scrollY) * 64;
		_mushroom[i].x += (scrollX - _scrollX) * 64;
		// mushroom RECT 고정
