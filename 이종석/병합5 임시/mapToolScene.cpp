...
HRESULT mapToolScene::init()
{
	...
	// 캐릭터 선택 변수 (위가 너무 길어서)
	playerOn = OctopusOn = MushroomOn = BossOn = false;
	playerSet = octopusSet = mushroomSet = bossSet = gameStart = false;
...
void mapToolScene::update()
{
	...
	{
		//캐릭터 선택용 RECT
		_rcPlayer = RectMake(_rcUIBg.left + 130, _rcUIBg.top + 75, 160, 200);
		_rcMushroom = RectMake(_rcPlayer.right + 80, _rcUIBg.top + 75, 163, 92);
		_rcOctopus = RectMake(_rcPlayer.right + 80, _rcMushroom.bottom + 10, 163, 92);
		_rcBoss = RectMake(_rcMushroom.right + 80, _rcUIBg.top + 75, 160, 200);
	}
	...
	// 플레이어가 화면에 세팅 되었을때 발동됨. update로 camX, camY값을 받아서 화면이 움직이는 값만큼 움직인다
	// 게임 스타트가 되어도 계속 돌아감
	if (playerSet)
	{
		_player->update(camX, camY, gameStart, octopusSet, mushroomSet, bossSet);
	...
	if (octopusSet)
	{
		_octopus->update(camX, camY, gameStart, playerSet);
	}
	if (mushroomSet)
	{
		_mushroom->update(camX, camY, gameStart, playerSet);
	}
	if (bossSet)
	{
		_boss->update(camX, camY, gameStart, playerSet);
	}
...
void mapToolScene::render()
{
	...
	if (playerOn)
	{
		IMAGEMANAGER->alphaRender("플레이어앞모습", getMemDC(), _ptMouse.x, _ptMouse.y, 100);
	}
	if (OctopusOn)
	{
		IMAGEMANAGER->alphaRender("red", getMemDC(), _ptMouse.x, _ptMouse.y, 0, 0, 80, 64, 100);
	}
	if (MushroomOn)
	{
		IMAGEMANAGER->alphaRender("mushroom", getMemDC(), _ptMouse.x, _ptMouse.y, 30, 40, 128, 88, 100);
	}
	if (BossOn)
	{
		IMAGEMANAGER->alphaRender("보스기본", getMemDC(), _ptMouse.x, _ptMouse.y, 100, 250, 284, 262, 100);
	}
...
void mapToolScene::input()
{
	...
	if (INPUT->getKeyDown(VK_LBUTTON))
	{
		...
		//캐릭터 선택해서 XXXXOn되는 bool변수가 켜졌을때 마우스왼쪽 버튼 누르면 세팅이 되고 XXXXSet이 true로 변함
		...
		if (!PtInRect(&_rcUIBg, _ptMouse))
		{
			if (playerOn)
			{
				if (!playerSet)
				{
					_player = new player;
					POINT tmp;
					tmp.x = _ptMouse.x + (camX * 64);
					tmp.y = _ptMouse.y + (camY * 64);
					_player->init(tmp);
					_inGameSceneAdress->setPlayerPt(tmp);
					playerSet = true;
					playerOn = false;
				}
			}
			else if (OctopusOn)
			{
				if (!octopusSet)
				{
					...
					_octopus->init(_ptMouse.x, _ptMouse.y);
					octopusSet = true;
				}
				else
				{
				...
				_inGameSceneAdress->setOctopusPtX(_ptMouse.x);
				_inGameSceneAdress->setOctopusPtY(_ptMouse.y);
			}
			else if (MushroomOn)
			{
				if (!mushroomSet)
				{
					...
					_mushroom->init(_ptMouse.x, _ptMouse.y);
					mushroomSet = true;
				}
				else
				{
				...
				_inGameSceneAdress->setMushroomPtX(_ptMouse.x);
				_inGameSceneAdress->setMushroomPtY(_ptMouse.y);
			}
			else if (BossOn)
			{
				if (!bossSet)
				{
					_boss = new boss;
					_boss->init(_ptMouse.x, _ptMouse.y);
					bossSet = true;
				}
				else
				{
				...
				_inGameSceneAdress->setBossPtX(_ptMouse.x);
				_inGameSceneAdress->setBossPtY(_ptMouse.y);
			}
		}


		//캐릭터 렉트 선택 변수 (MENU_CHAR에서)
		if (PtInRect(&_rcPlayer, _ptMouse) && !playerSet)
		{
			playerOn = true;
			OctopusOn = false;
			MushroomOn = false;
		...
		else if (PtInRect(&_rcOctopus, _ptMouse) && !OctopusOn)
		{
			playerOn = false;
			OctopusOn = true;
			MushroomOn = false;
		...
		else if (PtInRect(&_rcMushroom, _ptMouse) && !MushroomOn)
		{
			playerOn = false;
			OctopusOn = false;
			MushroomOn = true;
		...
		else if (PtInRect(&_rcBoss, _ptMouse) && !bossSet)
		{
			playerOn = false;
			OctopusOn = false;
			MushroomOn = false;
	...
	{
		// 문어랄 버섯은 선택된 후 RBUTTON을 눌러야 꺼짐
		if (OctopusOn || MushroomOn)
		{
			OctopusOn = false;
			MushroomOn = false;
		}
