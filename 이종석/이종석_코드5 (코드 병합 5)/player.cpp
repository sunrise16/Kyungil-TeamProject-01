...
// 업데이트 (update)
void player::update(int _scrollX, int _scrollY, bool isStart, bool octopusSet, bool mushroomSet, bool bossSet)
{
	playerCenter.x = playerRc.left + 32;
	playerCenter.y = playerRc.top + 32;

	playerCenter.x += (scrollX - _scrollX) * 64;
	playerCenter.y += (scrollY - _scrollY) * 64;
	
	// 플레이어 RECT 업데이트
	if (playerRc.left <= 0 && playerRc.right <= WINSIZEX && playerRc.top <= 0 && playerRc.bottom <= WINSIZEY)
	{
		playerRc = RectMakeCenter(playerCenter.x, playerCenter.y, 64, 64);
	}
	else
	{
		...
		if (INPUT->getKey(VK_RIGHT) && playerRc.right > WINSIZEX)
		{
			playerCenter.x += 5;
			playerRc.left += 5;
			playerRc.right += 5;
		}
		if (INPUT->getKey(VK_UP) && playerRc.top > 0)
		{
			playerCenter.y -= 5;
			playerRc.top -= 5;
			playerRc.bottom -= 5;
		}
		if (INPUT->getKey(VK_DOWN) && playerRc.bottom > WINSIZEY)
		{
			playerCenter.y += 5;
			playerRc.top += 5;
			playerRc.bottom += 5;
		}
		playerRc = RectMake(playerRc.left, playerRc.top, 64, 64);
	}
	...
	if (isStart)
	{
		...
		if (playerGetAtk == false && playerGetAtkDelayBool == false && hpCount > 0)
		{
			...
			// 오른쪽 공격
			if (INPUT->getKey(VK_SPACE) && playerRightStanding == true)
			{
				// 오른쪽 공격하는 상태로 변경
				playerRightAtk = true;
				playerState = PLAYER_STATE_RIGHTATK;
			...
			// 위쪽 공격
			if (INPUT->getKey(VK_SPACE) && playerUpStanding == true)
			{
				// 위쪽 공격하는 상태로 변경
				playerUpAtk = true;
				playerState = PLAYER_STATE_UPATK;
			...
			// 아래쪽 공격 
			if (INPUT->getKey(VK_SPACE) && playerDownStanding == true)
			{
				// 아래쪽 공격하는 상태로 변경
				playerDownAtk = true;
				playerState = PLAYER_STATE_DOWNATK;
