...
{
	// 보스 이미지 추가
	_bossIdle = IMAGEMANAGER->addFrameImage("보스기본", "Images/Enemy/Boss/보스기본이미지 12288 + 512 _ X32 Y1.bmp", 9216, 384, 32, 1, true, RGB(255, 0, 255));
	_fireBall = IMAGEMANAGER->addFrameImage("파이어볼시전", "Images/Enemy/Boss/파이어볼시전 2496 + 480 _ X6 Y1.bmp", 1872, 360, 6, 1, true, RGB(255, 0, 255));
	_teleportS = IMAGEMANAGER->addFrameImage("텔레포트시작", "Images/Enemy/Boss/텔레포트시전 3584 + 480 _ X7 Y1.bmp", 2688, 360, 7, 1, true, RGB(255, 0, 255));
	_teleportE = IMAGEMANAGER->addFrameImage("텔레포트도착", "Images/Enemy/Boss/텔레포트도착 4096 + 480 _ X8 Y1.bmp", 3072, 360, 8, 1, true, RGB(255, 0, 255));//_eyeShield = IMAGEMANAGER->addFrameImage("아이쉴드", "Images/Enemy/Boss/아이쉴드 1280 + 128 _ X10 Y1.bmp", 1280, 128, 10, 1, true, RGB(255, 0, 255));
...
// 업데이트 (update)
void boss::update(int _scrollX, int _scrollY, bool isStart, bool playerSet)
{
	for (int i = 0; i < _boss.size(); i++)
	{
		//_boss[i]._hp -= 1;			// 죽었을 때 모션 확인
		...
		_boss[i].y += (scrollY - _scrollY) * 64;
		_boss[i].x += (scrollX - _scrollX) * 64;
		_boss[i]._rc = RectMakeCenter(_boss[i].x, _boss[i].y + 30, 300, 400);	// 렉트 위치 이미지 기준 업데이트
