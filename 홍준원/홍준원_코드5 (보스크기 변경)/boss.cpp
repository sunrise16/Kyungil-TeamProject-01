...
HRESULT boss::init()
{
	...
	_teleportE = IMAGEMANAGER->addFrameImage("텔레포트도착", "Images/Enemy/Boss/텔레포트도착 4096 + 480 _ X8 Y1.bmp", 3072, 360, 8, 1, true, RGB(255, 0, 255));
...
void boss::update()
{
	...
	if (INPUT->getKeyDown(VK_F1))
	{
		...
		boss._rc = RectMakeCenter(boss.x, boss.y + 30, 200, 290);		// 이미지 여백이 꽤 커서 렉트를 줄임;
		boss._hp = BOSS_HP;
		boss._angle = -(PI_2);
		boss._speed = 3.0f;
	...
	for (int i = 0; i < _boss.size(); i++)
	{
		...
		if (_boss[i]._hp > 0)
		{
			...
			_boss[i]._rc = RectMakeCenter(_boss[i].x, _boss[i].y + 30, 200, 290);	//렉트 위치 이미지기준 업데이트
			//버섯방향 (아래쪽)
...
void boss::render()
{
	for (int i = 0; i < _boss.size(); i++)
	{
		Rectangle(getMemDC(), _boss[i]._rc);
		if (_boss[i]._state == BOSS_IDLE)
		{
