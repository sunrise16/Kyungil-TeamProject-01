...
HRESULT player::init()
{
	...
	_playerLeft = IMAGEMANAGER->addFrameImage("플레이어왼쪽이동", "Images/Player/링크옆으로걷기_230+23_10장.bmp", 800, 80, 10, 1);
	_playerRight = IMAGEMANAGER->addFrameImage("플레이어오른쪽이동", "Images/Player/링크오른쪽옆으로걷기_230+23_10장.bmp", 800, 80, 10, 1);
	_playerUp = IMAGEMANAGER->addFrameImage("플레이어위쪽이동", "Images/Player/링크위로걷기_180+26_10장.bmp", 800, 80, 10, 1);
	_playerDown = IMAGEMANAGER->addFrameImage("플레이어아래쪽이동", "Images/Player/링크앞으로걷기_180+24_10장.bmp", 800, 80, 10, 1);
	...
	if (playerGetAtk == false && playerGetAtkDelayBool == false && hpCount > 0)
	{
		...
		if (INPUT->getKey(VK_LEFT) && playerRc.left >= 0)
		{
			...
			if (INPUT->getKey(VK_SPACE))
			{
				...
				playerGetAtkLeftRc = RectMakeCenter(playerRc.left, playerRc.top + 40, 10, 50);
				playerGetAtkRightRc = RectMakeCenter(playerRc.right, playerRc.top + 40, 10, 50);
				playerGetAtkTopRc = RectMakeCenter(playerRc.left + 40, playerRc.top, 50, 10);
				playerGetAtkBottomRc = RectMakeCenter(playerRc.left + 40, playerRc.bottom, 50, 10);
				playerAttack = RectMakeCenter(playerRc.left - 20, playerRc.top + 40, 10, 50);
			}
		...
		else if (INPUT->getKey(VK_RIGHT) && playerRc.right <= WINSIZEX)
		{
			...
			if (INPUT->getKey(VK_SPACE))
			{
				...
				playerGetAtkTopRc = RectMakeCenter(playerRc.left + 40, playerRc.top, 50, 10);
				playerGetAtkBottomRc = RectMakeCenter(playerRc.left + 40, playerRc.bottom, 50, 10);
				playerAttack = RectMakeCenter(playerRc.right + 20, playerRc.top + 40, 10, 50);
			}
			// 공격 키를 뗐을 때 달리는 상태로 다시 전환
			if (INPUT->getKeyUp(VK_SPACE))
			{
		...
		else if (INPUT->getKey(VK_UP) && playerRc.top >= 0)
		{
			...
			if (INPUT->getKey(VK_SPACE))
			{
				...
				playerGetAtkTopRc = RectMakeCenter(playerRc.left + 40, playerRc.top, 50, 10);
				playerGetAtkBottomRc = RectMakeCenter(playerRc.left + 40, playerRc.bottom, 50, 10);
				playerAttack = RectMakeCenter(playerRc.left + 40, playerRc.top - 20, 50, 10);
			}
			// 공격 키를 뗐을 때 달리는 상태로 다시 전환
			if (INPUT->getKeyUp(VK_SPACE))
			{
		...
		else if (INPUT->getKey(VK_DOWN) && playerRc.bottom <= WINSIZEY)
		{
			...
			if (INPUT->getKey(VK_SPACE))
			{
				...
				playerGetAtkTopRc = RectMakeCenter(playerRc.left + 40, playerRc.top, 50, 10);
				playerGetAtkBottomRc = RectMakeCenter(playerRc.left + 40, playerRc.bottom, 50, 10);
				playerAttack = RectMakeCenter(playerRc.left + 40, playerRc.bottom + 20, 50, 10);
			}
			// 공격 키를 뗐을 때 달리는 상태로 다시 전환
			if (INPUT->getKeyUp(VK_SPACE))
			{
